...
const getAIClient = () => {
  // In a static GitHub Pages environment, process.env might not be available at runtime
  // unless injected. We handle both cases to prevent crashes.
  try {
...
export const fetchEyeCareTip = async (): Promise<EyeCareTip> => {
  ...
  try {
    ...
    console.warn("EyeCare Tip: Using fallback (Check API Key in environment)", error);
    return fallbackTip;
  }
};
